...
CREATE TABLE public.campaigns (
  ...
  show_pecas_graficas BOOLEAN NOT NULL DEFAULT true,
  show_solicitacao_spot BOOLEAN NOT NULL DEFAULT true,
  show_material_explicativo BOOLEAN NOT NULL DEFAULT true,
...
CREATE POLICY "Users can view their own pecas graficas" 
ON public.pecas_graficas 
FOR SELECT 
USING (auth.uid() = user_id);

CREATE POLICY "Users can create their own pecas graficas" 
ON public.pecas_graficas 
FOR INSERT 
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own pecas graficas" 
ON public.pecas_graficas 
FOR UPDATE 
USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own pecas graficas" 
ON public.pecas_graficas 
FOR DELETE 
USING (auth.uid() = user_id);
...
-- Create RLS policies for solicitacao_spot
